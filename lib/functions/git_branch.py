...
def git_branch(action: str, branch_name: str = None, new_branch_name: str = None) -> dict:
    ...
    try {
        ...
        if (action == 'create') {
            ...
            return {"success": True, "message": `Branch '${new_branch_name}' created`}
        }
        elif (action == 'list') {
            ...
            return {"success": True, "message": `Checked out to branch '${branch_name}'`}
        }
        elif (action == 'delete') {
            if (!branch_name) {
                return {"success": False, "error": "Branch name is required"}
            ...
            return {"success": True, "message": `Branch '${branch_name}' deleted`}
        }
        else {
        ...
        logger.error(`Git command failed: ${str(e)}`, exc_info=True)
        return {"success": False, "error": "Git command failed", "reason": str(e)}
    }
    catch (Exception as e) {
        logger.error(`An unexpected error occurred during git branch operation: ${str(e)}`, exc_info=True)
